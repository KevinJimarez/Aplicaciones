# Aplicaciones

Repositorio académico de **Kevin Salas Jimarez** (matrícula **2311080876**) para la materia **Aplicaciones Web Progresivas**, grupo **10 B - 2026**.

## Actividades

| Núm. | Actividad | Contenido |
| --- | --- | --- |
| 01 | [Aplicación nativa para Wear OS](./01-aplicacion-nativa-wear-os/) | Código Android, recursos, capturas y reporte académico. |
| 02 | [Aplicación web NEXORA](./02-aplicacion-web-nexora/) | Código React, evidencias, instrucciones y reporte académico. |

Cada actividad se encuentra en su propia carpeta para mantener separados el código, las dependencias y la documentación de cada entrega.

## Datos académicos

- **Alumno:** Kevin Salas Jimarez
- **Matrícula:** 2311080876
- **Profesora:** Mather Xóchitl Mendoza Píscil
- **Asignatura:** Aplicaciones Web Progresivas
- **Grupo:** 10 B - 2026
